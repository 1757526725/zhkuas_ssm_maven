...
var ClassroomPage = (function() {
	...
	var _saveToDb = function(){
		Util.ajax.post(SITE_HOST+ "/admin/school/classrooms","JSON",{"step" : 2},
				function(data) {
					if (data.state == 1) {
						var addCount = data.result.addCount;
						var updateCount = data.result.updateCount;
						showInfo("结果","添加了"+ addCount+ "间，更新了"+ updateCount+ "间教室！",3000);
					}
		});
	}
	// 校区队列抓取 教学楼
	var _crawClassroomByQueue = function(queue, interval) {
		if (queue == null)
			return;
		if (interval < 0)
			interval = 0;
		if (queue.constructor == Array) {
			...
			if (queue.length == 0) {
				// 队列任务结束
				// 请求服务器 整理 数据
				Util.ajax.post(SITE_HOST + "/admin/school/classrooms","JSON",{"step" : 1},
					function(data) {
						if (data.state == 1) {
			...
			// 发动请求
			Util.ajax.put(SITE_HOST + "/admin/school/building/" + task.no
					+ "/classrooms", "JSON", null, function(data) {
				// 请求成功
				if (data.state == 1) {
				...
				setTimeout(function() {
					_crawClassroomByQueue(queue, interval);
				}, interval);
			}, function(error) {
				// 请求失败，将任务添加到未完成任务中
				_progressBar.addRemainTask(task);
